...
- OpenAPI is 103 paths. The newest generated path is public `GET /api/v1/cards/{id}/provenance`.
...
- Sports catalog, team search, slug lookup, and leaderboard join/leave are shipped. They are not in OpenAPI yet. `TeamFollow` stays social-only.
- `POST /api/locker-room` `authorTeamId` (alias `postAsTeamId`) is distinct from the about-tag `teamId` and returns `403` when unauthorized.
