---
title: "d-sports-api"
description: "PWA and backend for the D-Sports fan engagement platform. Next.js, Prisma, Clerk, RevenueCat, Thirdweb."
icon: "server"
---

## Overview

**d-sports-api** (package name: `d-sports-pwa`) is the main application: a Progressive Web App that provides the full fan engagement experience. It is both the backend and the web client.

- **Default port:** 3004
- **Run:** `bun dev`

## Tech stack

| Category   | Technology                    |
| ---------- | ----------------------------- |
| Framework  | Next.js 16, React 19          |
| Database   | PostgreSQL, Prisma 6.8        |
| Auth       | Clerk 6.x                     |
| Styling    | Tailwind CSS 4, Radix UI, shadcn/ui |
| Web3       | Thirdweb, Viem, Wagmi        |
| Payments   | RevenueCat (purchases-js)     |
| State      | TanStack Query, TanStack Store |
| Secrets    | HashiCorp Vault (optional)    |
| Package    | Bun                           |

## Features

- **Locker room** — Social feed, posts, comments, likes, following
- **Gamification** — Quests, points, achievements, leaderboards, Frontline Pass
- **Digital collectibles** — NFT-style collectibles, rarity tiers, pack opening, inventory
- **Shop** — Product catalog, cart, RevenueCat purchases
- **Wallet** — Web3 wallet, token balances, Thirdweb integration
- **Interactive games** — Trivia, wheel spin
- **Team manager dashboard** — Collectible creation, bulk upload, packs, analytics, revenue
- **Team management** — Superadmin team-manager assignment, per-team member roles (owner/editor/viewer), canonical team sync
- **Admin** — Impersonation, roles (SuperAdmin, Admin, TeamManager), user management

## What changed since last docs sync

- Team-management and admin route surfaces were expanded and hardened with RBAC layers.
- Team and league canonical sync behaviors were expanded, including webhook-triggered updates and admin sync controls.
- Quest/reward behavior evolved toward team-aware and pass-aware progression.
- Pack odds and compliance-oriented disclosure behavior received implementation and runbook updates.

## Getting started

1. Clone the repository and install dependencies (`bun install`).
2. Set up environment variables (see repo `.env.example` or Vault setup in README).
3. Configure PostgreSQL and run `bun prisma db pull` then `bun prisma generate`.
4. Run `bun dev` — app is at http://localhost:3004.

For full setup (Vault, seeds, migrations), see the repository README.

## API

This app is the primary API surface for the D-Sports platform. The [API reference](/api-reference/introduction) in these docs covers endpoints and usage; detailed OpenAPI or API specs live in the repo.

## Season 0.5 leaderboard infrastructure

- Points events are now scoped in `PointsHistory` by both leaderboard and season.
- Season reset/wipe behavior is unchanged.
- Legacy Season 0 rows with null scoping are intentionally preserved for backward compatibility.

### Leaderboard scoring behavior

- Team boards now compute totals from `PointsHistory` rows filtered by both active `seasonId` and the team `leaderboardId`.
- The global board computes totals from `PointsHistory` rows filtered by active `seasonId` only (cross-board season aggregate).
- New point writes (quests, onboarding, daily actions, backfill scripts) include `seasonId` and `leaderboardId` where available.
- Some historical/backfill paths can intentionally write `leaderboardId: null` when a board cannot be determined yet.
- Legacy Season 0 rows with null season/board scope remain in storage and are intentionally excluded from Season 0.5+ scoped calculations.

### API contract notes

- `GET /api/leaderboard?leaderboardId=<id>` resolves and scores a specific board.
- `GET /api/leaderboard?teamId=<teamId>` resolves (or creates) that team board, then scores it.
- `GET /api/leaderboard` (no params) returns global leaderboard scoring for the active season.
- This release does not introduce season reset/archiving behavior changes, and does not remove `LeaderboardEntry.points` yet (planned Season 2 cleanup).

## Team management

Superadmins can assign users with the `team_manager` role to specific teams with a scoped role (`owner`, `editor`, or `viewer`). Team managers then access a landing page at `/team-mgmt` scoped to their assigned teams.

### Role hierarchy

| Role | Scope | Can do |
| --- | --- | --- |
| **SuperAdmin** | Global | Assign/remove team managers to any team |
| **Owner** | Per-team | Manage members, change roles, remove members |
| **Editor** | Per-team | Edit team content (scoped by team permissions) |
| **Viewer** | Per-team | Read-only access to team dashboard |

### Admin API endpoints

- `GET /api/admin/team-assignments` — list all team-manager assignments
- `POST /api/admin/team-assignments` — assign a team manager to a team
- `PATCH /api/admin/team-assignments` — update an assignment role
- `DELETE /api/admin/team-assignments` — remove an assignment
- `GET /api/admin/teams` — list teams (all or `?my=true` for assigned teams)
- `POST /api/admin/teams` — create a team
- `GET /api/admin/teams/{teamId}` — get a single team
- `GET /api/admin/teams/{teamId}/members` — list team members
- `POST /api/admin/teams/{teamId}/members` — add a team member
- `PATCH /api/admin/teams/{teamId}/members/{userId}` — update member role
- `DELETE /api/admin/teams/{teamId}/members/{userId}` — remove a member
- `POST /api/admin/teams/sync` — sync canonical teams and leaderboards

See the full [API reference](/api-reference/introduction) for request/response details.

## Gamification deep dives!

Comprehensive non-admin gamification docs now live in dedicated sections with subsections:

<CardGroup cols={2}>
  <Card title="Gamification docs hub" icon="trophy" href="/repositories/d-sports-api/gamification/index">
    Start here for architecture and behavior docs across quests, points, leaderboards, rewards, and achievements.
  </Card>
  <Card title="Backend coverage status" icon="clipboard-list" href="/repositories/d-sports-api/gamification/coverage-status">
    See exactly what is documented vs missing across all non-admin backend feature domains, not only gamification.
  </Card>
</CardGroup>

## Backend domain deep dives

Each non-admin backend domain now has dedicated folderized docs with architecture and behavior subsections.

<CardGroup cols={2}>
  <Card title="Auth and onboarding" icon="shield-check" href="/repositories/d-sports-api/auth-onboarding/index">
    Session/auth boundaries, profile lifecycle, and onboarding behavior.
  </Card>
  <Card title="Social and locker room" icon="messages-square" href="/repositories/d-sports-api/social-locker-room/index">
    Posts, comments, reactions, follow graph, and stream behavior.
  </Card>
  <Card title="Commerce and purchases" icon="shopping-bag" href="/repositories/d-sports-api/commerce/index">
    Catalog, checkout, purchases, dsports-cash, and entitlement flows.
  </Card>
  <Card title="Collectibles and packs" icon="package" href="/repositories/d-sports-api/collectibles/index">
    Pack lifecycle, collectible ownership, inventory, and minting paths.
  </Card>
  <Card title="Wallet and web3" icon="wallet" href="/repositories/d-sports-api/wallet-web3/index">
    Wallet provisioning, key access controls, and provider integration behavior.
  </Card>
  <Card title="Moderation and reporting" icon="shield-alert" href="/repositories/d-sports-api/moderation/index">
    User-facing report intake and moderation model lifecycle.
  </Card>
  <Card title="Platform and infrastructure" icon="server" href="/repositories/d-sports-api/platform-infra/index">
    Streams, sync routes, backfills, and non-admin operational patterns.
  </Card>
  <Card title="Backend coverage status" icon="clipboard-list" href="/repositories/d-sports-api/gamification/coverage-status">
    Track what is documented, partial, or not yet documented across all non-admin backend domains.
  </Card>
</CardGroup>

## Admin and team management surfaces (public-safe)

Admin and team-management docs include contributor-facing behavior and route contracts while excluding sensitive internal operations.

<CardGroup cols={2}>
  <Card title="Admin and team management" icon="shield-check" href="/repositories/d-sports-api/admin-team-management/index">
    RBAC model, route map, and public-safe implementation guidance.
  </Card>
  <Card title="Teams and leagues pipeline" icon="network" href="/repositories/d-sports-api/teams-leagues/index">
    Canonical data sync, webhook flow, and downstream API behavior.
  </Card>
</CardGroup>

## Compliance and legal: pack opening odds

Pack-opening disclosure, operations, and app-review evidence references:

<Card title="Compliance and legal docs" icon="scale" href="/repositories/d-sports-api/compliance/index">
  Backend implementation and policy-alignment guidance for randomized pack disclosures.
</Card>

<Card title="Ecosystem overview" icon="map" href="/repositories/ecosystem-overview">
  See how d-sports-api fits with the site, native app, and Mic'd Up.
</Card>
