...
- OpenAPI grew from 73 to 102 paths. New public contracts: mobile session exchange, locker-room post CRUD, featured collectibles, games catalog + daily-spin/guess-player/live-active, binders, custodial `sign-transaction`/`export`, `/v1/engage` daily quests, Fan Rep reads/earns, and D-Sports Direct grants.
...
- Fan Rep is a first-class ledger, not a client-computed score. `dsportsDirectRep` sums `repType = Direct` rows from admin grants.
