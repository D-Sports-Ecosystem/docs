...
- OpenAPI is **243** paths. Public and authenticated client helpers from the last inventory are now generated, and quest/Rep operations use the canonical `/api/v1/engage/*` prefix. `POST /api/games/daily-spin/reward` is in the spec as deprecated. `POST /api/v1/engage/player-challenge/complete` is published as retired (`410`).
