...
- OpenAPI is **194** paths. New since the last sync: Game Center `player-stats`, `top-performer`, `win-probability`, and iOS `live-activity`; `GET /api/teams/{id}/head-to-head`; `POST /api/feed/signals`; and `PUT /api/admin/players/{playerId}/availability`. Marketplace, KYC, the [Game Engine](/repositories/game-engine/index), leftover `/api/games/*` play routes, realtime discovery, and league standings were already published.
