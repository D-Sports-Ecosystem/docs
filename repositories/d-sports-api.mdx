...
- **Interactive games** — Team-selected catalog (daily spin, guess-the-player, live games) plus the [Game Engine](/repositories/game-engine/index) for Pick'em, Trivia, air hockey, and the locker-singularity idle ledger
...
- OpenAPI is **253** paths, synced from `d-sports-api` `main` (`e9a8d95`) plus `GET /api/user/all`. New published surfaces: HQ starred teams, `GET /api/teams/{id}`, the locker-singularity idle ledger, and wallet attestation at `/api/wallets/{address}/attestation` (the path value is the wallet database id). `PUT /api/packs/{id}/purchase` is in the spec as removed (`410`); confirm with `POST /api/checkout/crypto/verify`.
