...
- **Interactive games** — Team-selected catalog (daily spin, guess-the-player, live games) plus the [Game Engine](/repositories/game-engine/index) for Pick'em and Trivia
...
- OpenAPI is **136** paths. New generated surfaces include `GET /api/events/realtime` and `GET /api/leagues/{leagueId}/standings`, plus sports catalog, team search/join/roster, leagues, locker-room drafts/likes, Game Center `/api/events/*`, notifications, and settings.
- The [Game Engine](/repositories/game-engine/index) (`/api/v1/engage/games/*`) is shipped and consumed by native. It is not in generated OpenAPI yet.
...
<CardGroup cols={2}>
  ...
  <Card title="Game Engine" icon="blocks" href="/repositories/game-engine/index">
