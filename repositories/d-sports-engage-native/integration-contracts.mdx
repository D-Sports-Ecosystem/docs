...
- Interactive games (Daily Pick'Em, Spin Wheel, Guess the Player) consume endpoints via `lib/api/games-api.ts`.
- Game state, daily availability, and results are server-authoritative.
- Game completions contribute to quest progression and point awards through the backend.
