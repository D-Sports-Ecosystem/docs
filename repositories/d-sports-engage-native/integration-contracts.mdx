...
- `GET /api/leagues/{leagueId}/standings` is public HQ standings. `leagueId` is the Prisma id or slug. Rows arrive sorted. Empty rows mean the league cannot be scored honestly.
- `GET /api/teams/{id}/head-to-head` is public and returns `{ record, meetings }` for the five most recent decided meetings.
...
- Native also calls `GET /api/v1/engage/games/catalog` and joins the two lists. Engine session verbs, HUD rules, and update-required behavior live in [Game Engine](/repositories/game-engine/index). Do not ignore unknown engine keys.
...
- Detail, box score, player-stats, plays, lineup, top-performer, and win-probability are public reads under `/api/events/{gameId}`.
...
- `POST/DELETE /api/events/{gameId}/live-activity` registers or ends an iOS Live Activity. Requires `UserSettings.liveScoreUpdates` consent.
