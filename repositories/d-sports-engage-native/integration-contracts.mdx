...
- MFA (multi-factor authentication) is handled client-side through Clerk. The app supports multiple second-factor strategies — `email_code`, `phone_code`, `totp`, and `backup_code` — with `email_code` as the preferred default when available. The login flow detects which strategies the user has enrolled and selects accordingly.
