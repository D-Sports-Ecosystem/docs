...
## Push notifications and error tracking

- **OneSignal** is integrated via `onesignal-expo-plugin` and `react-native-onesignal` for push notification delivery and subscription management.
- **Sentry** (`@sentry/react-native`) provides crash reporting and error tracking across all platforms.
- **Supabase** client-side JS is used for realtime features — this is not a direct database connection.
