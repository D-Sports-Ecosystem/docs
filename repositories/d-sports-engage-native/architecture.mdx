...
- The leaderboard tab uses a nested Stack layout (`app/(tabs)/leaderboard/_layout.tsx`) with an `index` route for the team overview and a dynamic `[teamSlug]` route for per-team or global leaderboard screens.
