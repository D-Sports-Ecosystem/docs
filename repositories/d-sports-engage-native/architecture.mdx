...
- Leaderboard uses stack routes (`/leaderboard/[teamSlug]`) so each team has a dedicated screen that resolves the board by team id.
- Locker room games (Prize Wheel, Guess the Player) open as dedicated stack routes (`/locker-room/wheel-spin`, `/locker-room/player-guess`) instead of nested modals.
