...
- Game Engine calls use `lib/api/games-engine-api.ts` through `useGamesEngineApi()`, not the shared `useApi()` bag. See [Game Engine client](/repositories/game-engine/client).
