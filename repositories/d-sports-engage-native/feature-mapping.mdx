...
- `app/market` splits pack browsing (`market/packs`) from trading (`market/trading`) so one tab does not lose the other. Listings go through `/api/v1/marketplace/*` (in generated OpenAPI). `listings/mine` still works; `me/listings` is the live path.
