...
- `app/market` splits pack browsing (`market/packs`) from trading (`market/trading`) so one tab does not lose the other. Listings go through `/v1/marketplace/*` (not yet in generated OpenAPI). `listings/mine` still works; `me/listings` is the live path.
...
- Game Center reads `/api/events/schedule`, `/api/events/realtime`, and `/api/events/{gameId}` for hero, lineup, chat, presence, ratings, and Broadcast discovery.
