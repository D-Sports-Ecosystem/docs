...
- Email/password sign-in uses Clerk's combined sign-in-or-up flow with automatic second-factor challenge.
- MFA verification supports TOTP, SMS phone code, and backup code strategies — the app auto-detects which methods the user has enabled.
- OAuth sign-in (Google, Apple) is available alongside email/password and bypasses MFA (handled by the OAuth provider).
