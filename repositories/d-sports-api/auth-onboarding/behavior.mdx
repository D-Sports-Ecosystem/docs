...
- Username/handle input is normalized before storage: leading `@` characters are stripped so handles are stored without a prefix (see `normalizeHandleInput` in `lib/handle-utils.ts`).
