...
- `GET /api/teams/{id}/roster` is public and returns active `Player` rows for the lineup builder.
- `/api/leagues` returns league metadata and color/logo payloads.
- `GET /api/teams/{id}/head-to-head` is public and returns `{ record, meetings }` for the five most recent decided meetings from the team's perspective.
- `GET /api/leagues/{leagueId}/standings` returns already-sorted `{ rows, tiebreakRules }`. Prefer unique vendor rank; otherwise points, wins, then goal difference.
