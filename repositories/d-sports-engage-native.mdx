...
## iOS builds with Xcode Cloud

The repository includes a post-clone script at `ios/ci_scripts/ci_post_clone.sh` for Xcode Cloud builds. This script automatically runs `pod install` after Xcode Cloud clones the repo, using Bundler if a `Gemfile` is present or running CocoaPods directly otherwise.
