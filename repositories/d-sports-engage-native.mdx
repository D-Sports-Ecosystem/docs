...
- **Auth** — Email/password and OAuth (Apple, Google) login on a single screen; OTP verification for new accounts
...
- **Authentication consolidated on `/login`** — the login screen now supports email/password sign-in (and sign-up) alongside OAuth (Apple / Google). The separate `/sign-up` route redirects to `/login`.
- A new `useEmailPasswordAuth` hook handles the combined sign-in-or-up flow using Clerk Core 3 internal future APIs, with OTP email verification for new accounts.
- Help Center FAQ updated to reflect the new email/password auth flow.
