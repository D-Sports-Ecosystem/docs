...
**d-sports-engage-native** (package name: `engage-native`, current version: **1.15.0**) is the native mobile app for D-Sports. It mirrors the core PWA experience on iOS and Android: wallet, shop, leaderboard, locker room, and profile.
...
Only `EXPO_PUBLIC_*` keys are accessible at runtime in the Expo app. Create a `.env` file in the repo root with the following keys:
...
<Warning>
  Never commit `.env.local` or files containing secret keys. The `.env` checked into the repo contains only publishable/client-safe values.
</Warning>

## EAS build profiles and CI/CD

The project uses [Expo Application Services (EAS)](https://expo.dev/eas) for native builds and over-the-air (OTA) updates. Build profiles are defined in `eas.json`:

| Profile | Channel | Use case |
| ------- | ------- | -------- |
| `development` | `development` | Dev client builds with simulator support |
...
| `staging` | `staging` | Pre-production with auto-increment versioning |
| `production` | `production` | App Store / Play Store release builds |

### OTA updates

A GitHub Actions workflow (`.github/workflows/eas-update.yml`) publishes OTA updates via `eas update` on pushes to `develop` and `main` branches. The workflow uses Bun 1.3.9 and Node 22 for EAS tooling.
...
2. Copy `.env` or create your own with the required `EXPO_PUBLIC_*` variables listed above.
