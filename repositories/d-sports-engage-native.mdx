...
When building for iOS, ensure the Apple Pay Merchant ID in `ios/DSportsEngage/DSportsEngage.entitlements` matches the value in `app.json` (`merchant.com.dsports.wallet.beta`). A mismatch causes provisioning profile errors for `com.apple.developer.in-app-payments`.
