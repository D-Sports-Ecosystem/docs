...
- **v1.17.1** — Per-team leaderboard stack routes (`/leaderboard/[teamSlug]`) resolve boards by `teamId` so rankings match the server. MMKV cache shape was fixed so opening a team detail modal before the full leaderboard no longer shows an empty list. Teams without league data (e.g. House of Doge) display an "Official" label instead of a generic fallback.
- **v1.17.0** — Prize Wheel and Guess the Player open as dedicated stack routes from Locker Room. Points history includes game awards. Quest list refreshes on tab focus. Followed teams sort first in Explore Teams. Profile follow count syncs after follow/unfollow. Leaderboard modal closes before navigating to a user profile.
...
- `GET /api/leaderboard` now accepts `teamId` as the preferred query param; `leaderboardId` is still supported but `teamId` takes precedence so the canonical per-team board is always returned.
