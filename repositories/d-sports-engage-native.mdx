...
- **Accessibility** — Full Apple Accessibility Nutrition Label compliance (7/9 categories supported, 2 planned)

## What changed since last docs sync

- **Full Apple Accessibility Nutrition Label support (v1.15.0)** — all 7 applicable categories (VoiceOver, Voice Control, Larger Text, Dark Interface, Differentiate Without Color, Sufficient Contrast, Reduce Motion) are fully supported. Captions and Audio Descriptions are planned for when video content is introduced.
- **Server-synced accessibility settings** — in-app accessibility preferences now sync to the backend via `AccessibilitySettingsSync`, so preferences persist across devices.
