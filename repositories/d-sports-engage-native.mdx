...
**Xcode Cloud** handles iOS builds. The `ios/ci_scripts/ci_post_clone.sh` script runs after cloning to set up the build environment:

- Installs a local Node.js (v20.18.0) if not already present—required for CocoaPods autolinking
- Runs `pod install` via Bundler (if available) or directly
