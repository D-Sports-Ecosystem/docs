...
- **Locker room** — Social feed, Top Fans leaderboard, and engagement
...
**v1.16.2**
- **Top Fans leaderboard** now uses real API data: `points`, `level`, `fans` (follower count), and `rank` replace the previous `xp`-based placeholders.
- `profileUrl` is the preferred avatar field on `TopUser`; `avatar` is deprecated.
- Web carousels for Top Fans and Top Teams gained mouse-drag scrolling and dot-based page navigation.
- Report-and-hide UX now shows a success toast before the hide prompt, with a configurable toast duration constant.
