...
| ------ | -------- | -------- |
| Fiat   | RevenueCat | Apple IAP (native), Google Play (native), Stripe (web) |
| Crypto | Thirdweb | Arbitrum (default), Ethereum, Polygon |

The crypto checkout flow calls the PWA backend at `POST /api/checkout/crypto` and `POST /api/checkout/crypto/verify`.
