...
- **Locker room owner actions** — post and comment authors can now edit or delete their own content from the feed action menu.
- **Cross-platform sharing** — posts and comments can be shared via permalink. On native the OS share sheet opens; on web the link is copied to the clipboard.
- **Deep-link routing** — permalink URLs (`/locker-room/post/{postId}` and `/locker-room/post/{postId}/comment/{commentId}`) resolve into the locker room tab and auto-focus the linked content.
