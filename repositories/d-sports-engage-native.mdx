...
- **Achievement badges redesigned** — profile achievements now use a shared `ProfileAchievement` model and reusable components (`AchievementCard`, `AchievementChip`, `AchievementsSection`) across own profile, public profile, and the achievements modal.
- **Fan level badge system added** — a points-based fan level badge (Rookie, Rising, All-Star, Legend, Mythic) is computed client-side and rendered alongside earned badges.
- **Responsive achievement grid** — achievement cards use adaptive column sizing for mobile and desktop instead of fixed widths.
