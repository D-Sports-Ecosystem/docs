...
| Auth       | Clerk (`@clerk/clerk-expo`) — Google, Apple, email/password, MFA |
...
- **Game Engine** — Shared shell for Daily Pick'em and Daily Trivia (`/(tabs)/locker-room/games/[gameKey]`)
...
- Stack is **Expo 57 / RN 0.86**. App version is **1.24.4**.
- The [Game Engine](/repositories/game-engine/client) ships Pick'em and Trivia inside one dynamic route. Daily Spin and Guess the Player stay on legacy `/api/games/*` routes.
