...
| Platform | Identifier |
| -------- | ---------- |
| Android  | `com.dsports.engage` |
| iOS      | `com.dsports.wallet.beta` |

## Getting started

1. Clone the repository and run `bun install` (pinned to `bun@1.3.9` via `packageManager`).
2. Copy `.env` and configure the required `EXPO_PUBLIC_*` variables:
...
3. Run `bunx expo start`, then press **a** for Android or **i** for iOS.
