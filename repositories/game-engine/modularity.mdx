...
- v1 does not include a game loop, remote game logic, or realtime. Physics stays outside engine core.
...
A catalog key with no registry entry is **Not in this build**. A registry entry whose `minClientVersion` is newer than the binary is **Update required**. Both are valid, visible states.
