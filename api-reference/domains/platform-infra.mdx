...
- Behavioral feed ingest: `POST /api/feed/signals` (authenticated). The server drops the payload when `UserSettings.analyticsPersonalization` is false. The generated spec does not yet document the request body.
