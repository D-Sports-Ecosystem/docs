...
- Behavioral feed ingest: `POST /api/feed/signals` (authenticated). The server drops the payload when `UserSettings.analyticsPersonalization` is false. The generated spec has no request body. The handler accepts unknown JSON into a persist that is not live yet, so there is no stable payload to document.
