...
| `POST` | `/api/v1/engage/games/{gameKey}/session/{sessionId}/action` | Bearer | `{ input, sequenceNumber? }`. Server returns `{ outcome, session? }`. If `session` is omitted, keep the session you already have. |
