...
| --- | --- | --- | --- |
| `GET` | `/api/v1/engage/games/catalog` | Public | Query `teamId` or `teamSlug`. Omit both for the platform catalogue. Optional `X-Client-Version`. Returns `{ daily, weekly }` rows that include `playable` and `reason`. |
