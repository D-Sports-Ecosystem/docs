...
Current intentional gaps in generated OpenAPI are admin/debug/internal operations plus marketplace catch-all and KYC. Game Engine and leftover public `/api/games/*` play routes are published. Deprecated `POST /api/games/daily-spin/reward` stays excluded. Curated domain pages cover approved admin/team routes without exposing sensitive internals.
