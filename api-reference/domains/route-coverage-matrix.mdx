...
| `/api/events/*` | Publish | Game Center schedule, realtime discovery, detail, stats, plays, lineup, chat, mute, presence, ratings. |
...
| `/api/v1/engage/games/*` | Shipped, OpenAPI pending | Game Engine catalog + session/action/complete/forfeit. Native already consumes this. See [Game Engine API](/api-reference/domains/game-engine). |
| `/api/games/guess-player/players`, `/session/invalidate`, `/api/games/live/*` (start/join/score/end/leaderboard), `/api/games/pick-em*`, `/api/games/daily-spin/reward` | Shipped, OpenAPI pending | Additional legacy play routes. `/reward` is deprecated. Engine Pick'em does not use `/api/games/pick-em*`. |
...
Current intentional gaps in generated OpenAPI are admin/debug/internal operations plus Game Engine `/api/v1/engage/games/*`, marketplace catch-all, KYC, and remaining live play routes. Curated domain pages cover approved admin/team routes and those pending public contracts without exposing sensitive internals.
