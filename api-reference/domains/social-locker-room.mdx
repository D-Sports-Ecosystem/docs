...
- Social graph: `/api/user/follow`, `/api/user/followers`, `/api/user/search`, `/api/user/all`, `/api/user/recommended`, `/api/user/top-users`
...
- `GET /api/user/all` is the authenticated community directory (newest onboarded users, excluding the caller). `limit` is 1–100 (default 50). `excludeFollowing=true` also drops people you already follow. `total` is the returned count. The response is `Cache-Control: no-store`, and the route allows 30 requests per minute.
